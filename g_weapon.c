...
void fire_shotgun2 (edict_t *ent)
{
	vec3_t right;
	vec3_t  forward;
	edict_t *barrel;
	...
	barrel2=G_Spawn();
	VectorClear(barrel2->s.origin);
	AngleVectors(ent->client->v_angle,forward,right,NULL);
	VectorMA(barrel->s.origin,50,right,barrel2->s.origin);
	SP_misc_explobox (barrel2);

	barrel3=G_Spawn();
	VectorClear(barrel3->s.origin);
	AngleVectors(ent->client->v_angle,forward,right,NULL);
	VectorMA(barrel->s.origin,100,right,barrel3->s.origin);
	SP_misc_explobox (barrel3);

	barrel4=G_Spawn();
	VectorClear(barrel4->s.origin);
	AngleVectors(ent->client->v_angle,forward,right,NULL);
	VectorMA(barrel->s.origin,-50,right,barrel4->s.origin);
	SP_misc_explobox (barrel4);

	barrel5=G_Spawn();
	VectorClear(barrel5->s.origin);
	AngleVectors(ent->client->v_angle,forward,right,NULL);
	VectorMA(barrel->s.origin,-100,right,barrel5->s.origin);
	SP_misc_explobox (barrel5);
	//end of spawn barrel BY Ninja of Comp
	G_FreeEdict (ent);
	G_FreeEdict (ent);
	G_FreeEdict (ent);
